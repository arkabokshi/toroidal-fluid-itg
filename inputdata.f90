MODULE inputdata
    ...
    REAL(KIND=dp),PARAMETER :: MinRad = 0.5_dp,                &
        ...
        shear  =  25.0_dp,                                   &    ! [1,30]
        CURV   =  1.0_dp,                                       &
        etag   =  1.16841088879567_dp,                                   &    ! [2,5]
        qedge  =  3.45_dp,                                      &    !

        etac  =  1062.5_dp,                                     &    !
        tau   =  1.0_dp,                                        &    !
        epsilonn  = 0.049739742362262_dp,                             &    ! [0.01,0.02,0.03,0.04,0.08]
        ...
        RunTime = 10000.1_dp,                                    &    !
        gammatol = 1e-4_dp,                                     &    ! Code will exit if the average change in gammae is below this value
        sigmatol = 1e-2_dp,
    ...
    REAL(KIND=dp) :: FlowOnOff = 1.0_dp,                       &    !
        ShearingRate,                                           &    !
        dGammaE_dt = 1.0e-1_dp
    ! If dynamically adjusting flow-shear, uncomment
    ! line 134 onwards in itg.f90 and make appropriate
    ...
    INTEGER :: MyRank, MySize, ierror, ModesPerProc                 ! MPI Variables
    ...
    CHARACTER(20) :: runpath = 'fig_35'                         ! The name of the path for file I/O.
